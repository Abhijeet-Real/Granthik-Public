...
logger.info("Starting GRANTHIK API (No Auth Mode)")
...
app = FastAPI(
    title="GRANTHIK API (No Auth Mode)",
    description="API for GRANTHIK document management and retrieval system - No Authentication Required",
    version="1.0.0",
)
...
# Monkey patch the dependencies to use no-auth versions
# This is a hack to avoid modifying all the route files
import app.api.deps
import app.api.deps_no_auth

# Replace the authentication dependencies with no-auth versions
app.api.deps.get_current_user = app.api.deps_no_auth.get_current_user
app.api.deps.get_current_active_user = app.api.deps_no_auth.get_current_active_user
app.api.deps.get_current_active_superuser = app.api.deps_no_auth.get_current_active_superuser
...
async def health_check():
    return {"status": "healthy", "mode": "no-auth"}